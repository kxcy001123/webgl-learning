...
function load(target, depth) {
  ...
  dirinfos.forEach((info) => {
    ...
    allFiles.push(path.join(__dirname, info));
...
app.get('/files', function (req, res, next) {
  ejs.renderFile('views/files.ejs', { files: allFiles }, function (err, data) {
    res.end(data);
  });
});
app.get('*', function (req, res, next) {
  var stats = fs.statSync(req.path);
  console.log(stats);

  res.send('文件大小：' + stats.size);
});
app.listen(8888);
